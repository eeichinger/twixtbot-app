...
class NeuralMCTS:
    ...
    def expand_leaf(self, game):
        ...
        if self.add_noise:
            ...
            leaf.P[leaf.LMnz] += self.add_noise * numpy.random.dirichlet(0.03 * numpy.ones(len(leaf.LMnz[0])))
    ...
    def visit_node(self, game, node, top=False, trials_left=-1):
        ...
        if top and self.smart_root:
            ...
            assert num_winnables > 0, (maxn, trials_left, numpy.array_str(node.N), numpy.array_str(node.LM))
            if num_winnables == 1:
        ...
        if top and self.verbosity > 1:
            print("selecting index=%d move=%s Q=%.3f P=%.5f N=%d" % (index, str(move), node.Q[index], node.P[index], node.N[index]))
    ...
    def top_moves_str(self, game):
        ...
        pts = [str(naf.policy_index_point(game, self.root.LMnz[0][index])) for index in indices[-3:]]
        return ":" + ",".join(pts)
    ...
    def mcts(self, game, trials):
        ...
        if self.root == None:
            ...
            if self.verbosity >= 1:
                ...
                print("eval=%.3f  %s" % (self.root.score, " ".join(["%s:%d" % (naf.policy_index_point(game, ix), int(self.root.P[ix]*10000+0.5)) for ix in top_ixs])))
        ...
        self.report = "%6.3f" % (self.root.Q[numpy.argmax(self.root.N)]) + self.top_moves_str(game)
        return self.root.N
