    ...
    def addsub(op):
        def f(self, other):
            if isinstance(other, Point):
                ...
                return Point(op(self.x, other[0]), op(self.y, other[1]))
            else:
                raise ValueError("Cannot add/subtract")
        return f
    __add__ = addsub(operator.add)
    __sub__ = addsub(operator.sub)

    def raddsub(op):
        def f(self, other):
            if len(other) == 2:
                return Point(op(other[0], self.x), op(other[1], self.y))
            else:
                raise ValueError("Cannot add/subtract")
        return f
    __radd__ = raddsub(operator.add)
    __rsub__ = raddsub(operator.sub)
...
class Game:
    ...
    def can_win(self, color):
        ...
        if color == Game.WHITE:
            ...
            endcheck = lambda pt: pt.y == Game.SIZE-1
            boundcheck = lambda pt: not pt.x in (0, Game.SIZE-1)
        else:
            assert color == Game.BLACK
            starts = [Point(0, y) for y in range(1, Game.SIZE-1)]
            endcheck = lambda pt: pt.x == Game.SIZE-1
            boundcheck = lambda pt: not pt.y in (0, Game.SIZE-1)
    ...
    def slow_legal_plays(self):
        ...
        if self.turn == Game.WHITE:
            xr = range(1, Game.SIZE-1)
            yr = range(0, Game.SIZE)
        else:
            assert self.turn == Game.BLACK
            xr = range(0, Game.SIZE)
            yr = range(1, Game.SIZE-1)
    ...
    def __str__(self):
        topline = "   " + "   ".join([chr(ord('A')+i) for i in range(Game.SIZE)]) + "\n"
        out = topline
        for y in range(Game.SIZE):
            if y > 0:
...
def get_thinker(spec, resources={}):
    ...
    if colon == -1:
        ...
        modname = spec[:colon]
        kwargs = {arg.split("=")[0]:arg.split("=")[1] for arg in spec[colon+1:].split(",")}
