...
class WorkTimeStat:
    ...
    def __str__(self):
        ...
        try:
            ...
            return "%s: N=%d T=%6g W=%8g%s  matrix error" % (self.name, N, T, W, batch_str)
        ratio_str = " a/b=%.1f" % (betas[0] / betas[1]) if betas[1] > 1e-9 else ""
        return "%s: N=%d T=%6g W=%8g%s avg=%.6f + %.6f*W%s" % (self.name, N, T, W, batch_str, betas[0], betas[1], ratio_str)
