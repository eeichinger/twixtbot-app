...
class NetInputs:
    ...
    def __init__(self, thing):
        ...
        elif isinstance(thing, (bytes, bytearray)):
    ...
    def hflip(self):
        ...
        self.recents = [twixt.Point(twixt.Game.SIZE-1-p.x, p.y) for p in self.recents]
    ...
    def vflip(self):
        ...
        self.recents = [twixt.Point(p.x, twixt.Game.SIZE-1-p.y) for p in self.recents]
    ...
    def _init_front_bytes(self, b):
        if b[:self.HEADER_BYTES] != self.HEADER:
            raise ValueError("Header sanity error (%s)" % (b[:self.HEADER_BYTES]))
        self.recents = []
        for i in range(self.NUM_RECENTS):
            x = b[self.HEADER_BYTES + i*2]
            y = b[self.HEADER_BYTES + i*2 + 1]
            if x == 255 and y == 255:
                continue
    ...
    def init_from_compact(self, b):
        ...
        assert abs(numpy.count_nonzero(p2[:,:,8]) - numpy.count_nonzero(p2[:,:,9])) <= 1
        self.naf[:,:,:10] = p2
    ...
    def init_from_expanded(self, b):
        ...
        if abs(c8-c9) > 1:
            print("Zut!! c8=%d, c9=%d", file=sys.stderr) % (c8, c9)
    ...
    def to_input_arrays(self, use_recents=False):
        ...
        if use_recents:
            locs = numpy.zeros((twixt.Game.SIZE, twixt.Game.SIZE, 3), dtype=numpy.float32)
...
def legal_move_policy_array(game):
    if game.turn == game.WHITE:
        pegsum = (game.pegs[0][1:game.SIZE-1,:] + game.pegs[1][1:game.SIZE-1,:]).flatten()
    else:
        assert game.turn == game.BLACK
        pegsum = (game.pegs[0][:,1:game.SIZE-1] + game.pegs[1][:,1:game.SIZE-1]).T.flatten()
    return 1 - pegsum
...
class LearningState:
    ...
    def from_bytes(b, name=None):
        ...
        ls.N = numpy.frombuffer(b, dtype=numpy.uint16, count=LearningState.NUM_MOVES, offset=LearningState.NAF_BYTES)
        ls.z = b[-1] - 1
        ls.name = name
        return ls
