...
class BattleSpec:
    def __init__(self, black_spec, white_spec, init_moves, with_training=True):
        ...
        self.move_list = list(init_moves) if init_moves else []
        self.win_color = None
...
def battle_once(black_th, white_th, moves, train_list, move_list=None):
    ...
    while not game.just_won():
        ...
        if isinstance(pmout, tuple) and not isinstance(pmout, twixt.Point):
...
class ThreadingManager:
    ...
    def job_maker_thread_run(self):
        ...
        while n < args.num_games:
            ...
            bs1 = BattleSpec(args.black, args.white, init_moves, args.training_file is not None)
            bs2 = BattleSpec(args.white, args.black, init_moves, args.training_file is not None)
...
if __name__ == "__main__":
    ...
    if args.threads:
        if args.display:
            print("Cannot use both display and threads.", file=sys.stderr)
            sys.exit(1)
        if args.show_moves:
            print("Cannot use both show_moves with threads.", file=sys.stderr)
            sys.exit(1)
