...
QUERY_AVAILABLE = b"QQ"
REPLY_AVAILABLE = b"RR"
ANSWER_SENT = b"SS"
...
class Client:
    ...
    def _request_slots(self, slots_needed):
        ...
        if debug:
            print("Sent request for %d slot(s)" % (slots_needed), file=sys.stderr)
        ...
        if debug:
            print("expecting %d bytes" % (4*(2+slots_needed)), file=sys.stderr)
        indata = self.socket.recv(4*(2+slots_needed), socket.MSG_WAITALL)
        if debug:
            print("received %d bytes" % (len(indata)), file=sys.stderr)
        ...
        if not self.quiet:
            print("query_size=", self.query_size, "reply_size=", self.reply_size,
                  "my_slots=[", ",".join(map(str, self.slots)), "]")
            sys.stdout.flush()
        # end _request_slots
    ...
    def write_query(self, request, reply_callback):
        if not isinstance(request, (bytes, bytearray)):
            raise ValueError('queries must be bytes')
        ...
        if debug:
            print("client sent %d bytes with checksum %d" % (len(request), cs), file=sys.stderr)
    ...
    def process_reply(self, slot):
        ...
        if debug:
            print("client received %d bytes with checksum %d" % (len(replydata), cs), file=sys.stderr)
        assert cs == self.shmem[y-3]
        cb(replydata)
    ...
    def handle_read(self):
        ...
        if debug:
            print("Client just read %d bytes" % (len(indata),), file=sys.stderr)
        if len(indata) % 4 != 0:
...
class ServerSocketProcess(multiprocessing.Process):
    ...
    def send_out_replies(self):
        ...
        for sock, outs in outs_by_socket.items():
            outdata = b''.join([IntPacker.pack(n) for n in outs])
...
class Server:
    ...
    def run_gpu_side(self):
        print("gpu side going!", file=sys.stderr)
        ...
        while True:
            ...
            if debug:
                print("timeout=", timeout, file=sys.stderr)
            rd, _, _ = select.select([self.gpu_conn], [], [], timeout)
            for sock in rd:
                x = sock.recv()
                if x == b"DIE" or len(x) == 0:
    ...
    def prepare_run_and_reply(self):
        if debug:
            print("jobs", ",".join(map(str, self.job_slots)), file=sys.stderr)
        ...
        for slot in self.job_slots:
            ...
            cs = checksum(wtd)
            if debug:
                print("server received %d bytes with checksum %d" % (len(wtd), cs), file=sys.stderr)
            assert cs == self.shmem[y-3], (cs, self.shmem[y-3])
            work.append(wtd)

        self.preproc_timer.stop()
        self.gpu_timer.start(len(self.job_slots))
        if debug:
            print("about to call self.run_jobs", file=sys.stderr)
        replies = self.run_jobs(work)
        if debug:
            print("finished with self.run_jobs", file=sys.stderr)
        self.gpu_timer.stop()
        ...
        for reply, slot in zip(replies, self.job_slots):
            ...
            if debug:
                print("server sent %d bytes with checksum %d" % (len(reply), cs), file=sys.stderr)

        self.pp_shm_timer.stop()
        self.pp_sock_timer.start(len(self.job_slots))
        self.gpu_conn.send_bytes(b"GO")
...
class SillyServer(Server):
    def run_jobs(self, jobs):
        return [bytes(reversed(job)) for job in jobs]
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        C = Client(sys.argv[1])
        C.write_query(sys.argv[2].encode(), cb)
        C.handle_read()
    else:
        print("usage: %s location [send_message]" % (sys.argv[0]), file=sys.stderr)
